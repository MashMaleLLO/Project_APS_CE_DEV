...
function OutputCareer() {
  ...
  const filteredCareerData = Object.entries(careerData).reduce((acc, [career, data]) => {
  ...
  return (
    <>
      <div>
        <button onClick={() => handleButtonClick("staticResult")}>Static Result</button>
        <button onClick={() => handleButtonClick("staticPredictResult")}>Static Predict Result</button>
      </div>
